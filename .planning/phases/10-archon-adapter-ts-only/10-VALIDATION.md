...
- **Max feedback latency:** ~15 seconds for unit + contract; the `gh pr list` check is external and gated by plan 10-06 checkpoint
...
| 10-03-02 | 03 | 2 | ARC-05 | unit (drift, 25 keys, 11/5/4/5 buckets) | `cd adapter-archon && pnpm test` | Wave 0 creates | pending |
...
| 10-05-02 | 05 | 4 | ARC-07 | YAML parse | python yaml.safe_load + grep anchors | Wave 0 creates | pending |
| 10-06-01 | 06 | 4 | ARC-08 | structural | file-exists + grep checks | Wave 0 creates | pending |
| 10-06-02 | 06 | 4 | ARC-08 | integration (external) | `gh pr list --repo coleam00/Archon --head gemini-sdk-integration --state open` | external | checkpoint |

*Status: pending = awaiting execution; green = automated command passes; red = failing; checkpoint = human-verify gate.*

Note: 10-03's drift test is pinned to the authoritative bucket counts from spec/archon/mapping.md: **11 honored / 5 partial / 4 deferred / 5 ignored = 25 keys** (corrected from prior 24-key inconsistency in 10-01 Task 3).
...
- [ ] `spec/archon/mapping.md` — canonical options triage, 25-row table, counts 11/5/4/5 (plan 10-01 Task 3)
...
- [ ] `adapter-archon/src/options-translator.ts` + `.spec.ts` (plan 10-03; drift test asserts 25 keys + 11/5/4/5 bucket counts)
...
- [ ] `.github/workflows/archon-contract.yml` + `archon-drift.yml` (plan 10-05 Task 2)
- [ ] `.planning/phases/10-archon-adapter-ts-only/pr-artifacts/` bundle — 9 files (plan 10-06 Task 1)
- [ ] Draft PR open at coleam00/Archon (plan 10-06 Task 2, human-verified checkpoint)
...
## Wave Structure (post-split)

| Wave | Plans | Parallelism |
|------|-------|-------------|
| 1 | 10-01, 10-02 | parallel (disjoint files) |
| 2 | 10-03 | depends on 10-01 (types, mapping.md) + 10-02 (lint script) |
| 3 | 10-04 | depends on 10-03 (translator) |
| 4 | 10-05, 10-06 | parallel; both depend on 10-04; 10-05 also depends on 10-02 for lint |

Plan 10-05 (contract test + CI workflows, ~6 files) and plan 10-06 (PR artifact bundle + draft-PR checkpoint, ~9 files) were split from the previous combined 10-05 (~15 files over blocker threshold).
...
- [x] Plan 10-05/10-06 split resolves scope-sanity warning (≤9 files per plan)
- [x] Plan 10-03 depends_on includes both 10-01 and 10-02

**Approval:** pending (awaits plan 10-06 checkpoint completion)
